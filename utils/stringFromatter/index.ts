...
export const stringFormatter = {
  ...
  firstLetterCapitalized: (str: string) => str.replace(/\b\w/g, char => char.toUpperCase()),
  ...
  camelCasePlural: (str: string) => {
    const normalized = str.toLowerCase().trim();
    const formatted = normalized.replace(/[-_\s]+(.)/g, (_, char) => char.toUpperCase());
    return formatted.endsWith('s') ? formatted : formatted + 's';
  },
  ...
  pascalCase: (str: string) => {
    const normalized = str.toLowerCase().trim();
    return normalized.charAt(0).toUpperCase() + normalized.slice(1).replace(/[-_\s]+(.)/g, (_, char) => char.toUpperCase());
  },
  
  // Convert to plural pascal case (doble cama -> DobleCamas, basica -> Basicas)
  pascalCasePlural: (str: string) => {
    const normalized = str.toLowerCase().trim();
    const formatted = normalized.charAt(0).toUpperCase() + normalized.slice(1).replace(/[-_\s]+(.)/g, (_, char) => char.toUpperCase());
    return formatted.endsWith('s') ? formatted : formatted + 's';
  },
  ...
  categoryFormatter: (str: string, format: 'title' | 'kebab' | 'snake' | 'camel' | 'pascal' | 'lowercase' | 'uppercase' = 'title', plural: boolean = false) => {
    // Normalize the input
    ...
    const categoryMappings: Record<string, { singular: string; plural: string; isPlural: boolean }> = {
      ...
      'estandares': { singular: 'estándar', plural: 'estándares', isPlural: true },
      'cama doble': { singular: 'cama doble', plural: 'camas dobles', isPlural: false },
      'camas dobles': { singular: 'cama doble', plural: 'camas dobles', isPlural: true },
      ...
      'familiares': { singular: 'familiar', plural: 'familiares', isPlural: true },
      'ejecutiva': { singular: 'ejecutiva', plural: 'ejecutivas', isPlural: false },
      'ejecutivas': { singular: 'ejecutiva', plural: 'ejecutivas', isPlural: true }
    };
    
    // Get the category mapping or create a fallback with proper Spanish plural rules
    const categoryMapping = categoryMappings[normalized] || (() => {
      let singular: string;
      let plural: string;
    ...
    const finalCategory = plural ? categoryMapping.plural : categoryMapping.singular;
    
    // Apply the requested format
    switch (format) {
      case 'title':
        return finalCategory.replace(/(^|\s)([a-záéíóúüñ])/g, (_, prefix, char) => prefix + char.toUpperCase());
      case 'kebab':
        return finalCategory.replace(/\s+/g, '-');
      case 'snake':
        return finalCategory.replace(/\s+/g, '_');
      case 'camel':
        return finalCategory.replace(/\s+(.)/g, (_, char) => char.toUpperCase());
      case 'pascal':
        return finalCategory.charAt(0).toUpperCase() + finalCategory.slice(1).replace(/\s+(.)/g, (_, char) => char.toUpperCase());
      case 'uppercase':
